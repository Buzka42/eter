...
/// An engraved gauge: a fine track ringed with measure ticks and a thin swept
/// arc. Replaces the 6px stroked ring with its sky-to-gold gradient.
class EngravedGauge extends StatelessWidget {
  const EngravedGauge({
    super.key,
          ...
          child: TweenAnimationBuilder<double>(
            ...
            builder: (context, p, _) => CustomPaint(
              ...
              child: Center(
                child: Text(
                  value,
                  style: text.titleMedium?.copyWith(color: ink.label),
                ),
              ),
            ),
          ),
...
class _GaugePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Twelve measure ticks, longer at the quarters — a dial face, not a bar
    // bent into a circle.
    final tick = Paint()
      ..color = line
      ..strokeWidth = 1
    ...
    for (var i = 0; i < 12; i++) {
      ...
      final inner = radius + (i % 3 == 0 ? -3.0 : 1.0);
      canvas.drawLine(
    ...
    // A small index mark at the swept end, the way a pointer terminates.
    final endAngle = -math.pi / 2 + eterSweepFor(progress);
    canvas.drawCircle(
